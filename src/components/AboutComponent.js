...
// the Render Leader Function
function RenderLeader({ leader }) {
  // pass the leader parameter
  return (
    <div key={leader.id} className="col-10 mt-5">
...
function About(props) {
  // map every leader variable and return a renderleader component with a parameter leader passed
  ...
  if (props.leadersLoading) {
    return <Loading />;
  } else if (props.leadersErrMsg) {
    // not loading but error
    return <h4>{props.leadersErrMsg}</h4>;
  } else {
    return (
